...
Tests mirror the source structure. The `tests/strategies/` subdirectory contains per-strategy tests (`test_ast_impact.py`, `test_pytest_impact.py`, `test_composite_impact.py`, `test_dependency_file_impact.py`, `test_caching.py`, `test_integration.py`). Extension system tests are in `tests/test_extensions.py` (unit tests) and `tests/test_extension_integration.py` (pytester integration). Monorepo support is tested in `tests/test_workspace.py` (discovery/graph unit tests) and `tests/test_monorepo_cli.py` (CLI tests plus slow end-to-end tests that copy the fixture into a temp git repo). Tests use `unittest.mock` extensively and the `pytester` pytest plugin (enabled in `conftest.py`) for testing plugin behavior. Some tests are marked `@pytest.mark.slow`.

A checked-in mock monorepo fixture lives at `examples/monorepo/` (uv-workspace root; `libs/pkg-alpha` is src-layout with zero config to exercise inference, `libs/pkg-beta` is flat-layout with explicit config and depends on pkg-alpha). It doubles as a manual playground: `uv run impacted-packages --root-dir examples/monorepo`. It is excluded from the main repo's pytest collection via `[tool.pytest.ini_options] testpaths = ["tests"]`.
