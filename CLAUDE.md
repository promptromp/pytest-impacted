...
- **`DependencyFileImpactStrategy`**: Detects changes in dependency/config files (`uv.lock`, `requirements.txt`, `pyproject.toml`, `Pipfile`, `Pipfile.lock`, `poetry.lock`, `setup.py`, `setup.cfg`, plus the glob patterns `requirements/*.txt` and `requirements/**/*.txt`) and marks all test modules as impacted. Accepts custom patterns via constructor. Enabled by default; disable with `--no-impacted-dep-files`
...
- CI's `lint` job runs `uv run pre-commit run --all-files` with `SKIP=pytest` rather than re-listing individual checks, so `.pre-commit-config.yaml` is the single source of truth for what gets enforced and a `--no-verify` commit cannot bypass the hygiene hooks. Add a new check to the pre-commit config and CI picks it up automatically — do not add bare lint steps to the workflows.
