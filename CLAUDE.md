...
**Lifecycle hooks**: `ImpactStrategy` exposes optional `setup(*, ns_module, tests_package, root_dir, session, dep_tree)` and `teardown()` methods with no-op defaults. `api.get_impacted_tests` calls `strategy.setup(...)` once before `strategy.find_impacted_tests(...)` and `strategy.teardown()` in a `finally` block after, so teardown always fires even if `find_impacted_tests` raises. `CompositeImpactStrategy` propagates `setup` in list order and `teardown` in reverse (LIFO); exceptions in any sub-strategy's hook are logged at WARNING level on `pytest_impacted.strategies` and swallowed so one misbehaving extension cannot prevent the others from running. Extensions doing O(source-tree) scans (e.g. pytest-impacted-microcosm's binding index) should move that work from lazy-init inside `find_impacted_tests` into `setup`.
